...
import { aaxEvents, setVerificationKey } from '../../src/helpers'
import {
    errorMessages,
    links,
...
import { redisInstance } from '../../src/libs'
    ...
    describe('Mutation: SignUp', () => {
        describe('signup new user', () => {
            ...
            test('User signed up should exist in the database', async () => {
                const exists = await photon.users.findOne({ where: { email: guestUserToCreate.email } })
    ...
    describe('Mutation: Login', () => {
        describe('login users', () => {
            ...
            test('should verify email upon link resolution', async () => {
                ...
                const user = await photon.users.findOne({ where: { email }, select: { emailVerified: true } })
    ...
    describe('Mutations: requestResetPassword, changePassword', () => {
        describe('create a change password request', () => {
            ...
            test('should lock the account, create a change request and send email to requestee', async () => {
                ...
                const pwd = await photon.users.findOne({ where: { email }, select: { password: true } })
                expect(pwd && pwd.password).toBe('*')
            })
        ...
        describe('change the password', () => {
            test('user should change the password', async () => {
                const { email } = guestUserToCreate
                const oldPassword = await photon.users.findOne({ where: { email }, select: { password: true } })
                const key = generateVerificationKey()
                await setVerificationKey(key, email)
                await aaxapiSdk.ChangePassword({ key, newPassword })
                const changedPassword = await photon.users.findOne({ where: { email }, select: { password: true } })
                expect(changedPassword && changedPassword.password).not.toBe(oldPassword && oldPassword.password)
            })
